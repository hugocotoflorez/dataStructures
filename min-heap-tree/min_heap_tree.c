...
int
int_lessthan(void *a, void *b)
{
    return *(int *) a < *(int *) b;
}
void
_raise(const char *errmsg)
{
...
void
_swap(struct HEAP_TREE *tree, int i, int j)
{
...
int
_get_parent(int current_index)
{
    return (current_index - 1) / 2;
}

int
_get_Lchild(int current_index)
{
    return current_index * 2 + 1;
}

int
_get_Rchild(int current_index)
{
...
void
_fix_topward(struct HEAP_TREE *tree, int current_index)
{
    element_t parent;
...
void
_fix_backward(struct HEAP_TREE *tree, int current_index)
{
    ...
    while (current_index <= tree->last_index)
    {
        ...
        if (childL > tree->last_index)
            break;
        smallest_child =
        tree->comp((void *) &tree->tree[childR], (void *) &tree->tree[childL]) ?
        childR :
        childL;
        if (tree->comp((void *) &tree->tree[smallest_child],
                       (void *) &tree->tree[current_index]))
        {
...
struct HEAP_TREE
create_heap_tree(int height, int (*comp)(void *, void *))
{
...
void
remove_heap_tree(struct HEAP_TREE *tree)
{
    free(tree->tree);
    tree->tree = NULL;
    if (MHT_VERBOSE)
...
void
add_heap_tree(struct HEAP_TREE *tree, element_t element)
{
...
void
print_tree(struct HEAP_TREE tree)
{
...
void
print_plain_tree(struct HEAP_TREE tree)
{
